 ...
 */
export async function ingestEuropePmc(env: Env): Promise<{ created: number; updated: number; total: number }> {
  const feed = await env.DB.prepare(`SELECT * FROM source_feeds WHERE id IN ('europe-pmc-batch', 'research-europe-pmc-rest') AND enabled = 1 LIMIT 1`).first<{
    id: string;
    endpoint_url: string;
  ...
  if (!feed?.endpoint_url) {
    throw new Error('Europe PMC feed missing or disabled');
  }
